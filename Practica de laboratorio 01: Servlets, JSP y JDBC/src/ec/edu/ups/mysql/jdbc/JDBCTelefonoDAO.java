...
public class JDBCTelefonoDAO extends JDBCGenericDAO<telefono, Integer> implements DaoTelefono {
    ...
    public telefono findById(Integer id) {
        ...
        try {
            if (rs != null && rs.next()) {
                phone = new telefono(rs.getInt("tel_id") , rs.getString("tel_numero"), rs.getString("tel_tipo"), rs.getString("tel_operadora"));
    ...
    public boolean delete(telefono phone) {
       return conexionDos.update("DELETE FROM telefono WHERE tel_id = '" + phone.getCodigo() + "' AND usu_cedula = '" + phone.getUser().getCedula() + "';");
    }
    ...
    public List<telefono> findByPersonaId(String cedula) {
        ...
        try {
            while (rs.next()) {
            	telefono phone = new telefono(rs.getInt("tel_id"), rs.getString("tel_numero"), rs.getString("tel_tipo"), rs.getString("tel_operadora") );
                phone.setUser(DAOFactory.getFactory().getUsuarioDAO().findById(rs.getString("usu_cedula")));
                phones.add(phone);
            }
        } catch (SQLException e) {
